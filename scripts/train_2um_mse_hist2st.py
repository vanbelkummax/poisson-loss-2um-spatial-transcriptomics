#!/usr/bin/env python3
"""
Model D': Virchow2 + Hist2ST Decoder + MSE Loss (Clean 2x2 Factorial)

This is the SIMPLE MSE baseline for the Hist2ST decoder.
...
EXPERIMENTAL DESIGN (2x2 factorial - FIXED):
    ...
    Hist2ST       | Model D' (this)      | Model E                 |
    Img2ST        | Model G              | Model F                 |

CRITICAL: This script uses IDENTICAL settings to Model G for fair comparison:
- Same loss: Simple masked MSE at 2µm only (NO multi-scale pyramid!)
- Same activation: Softplus for non-negative outputs
- Same supervision: 2µm only (no 8µm, 4µm, 16µm auxiliary losses)
- Same hyperparameters: lr=5e-5, batch=8, grad_accum=4, epochs=40

NOTE: This is DIFFERENT from the original Model D (train_conservation_v6.py):
- Original D uses multi-scale pyramid (8µm + 4µm + 16µm supervision)
- Original D uses log1p+Huber loss with auxiliary terms (cell, TV, GradNorm)
- This D' is a "stripped down" version for fair decoder comparison

KEY ARCHITECTURE:
- Encoder: Virchow2 (1.1B params, frozen)
- Decoder: Hist2ST (Transformer-based with spatial attention)
- Output: Softplus activation for non-negative predictions
- Loss: Simple MSE at 2µm (masked to tissue regions)

USAGE:
    python train_2um_mse_hist2st.py --test_patient P5 --save_dir results/model_d_prime_hist2st_mse
"""
...
class DecoderModel(nn.Module):
    """
    Encoder + Decoder for MSE loss at 2um (Model D').

    KEY: Uses Softplus activation for non-negative output (matches Model G).
    NO bias initialization to -3.0 (that's only for Poisson log_rate).
    """
    ...
    def __init__(self, encoder_name, decoder_type, num_genes=50, input_size=224):
        ...
        if decoder_type == 'miniunet':
            ...
            # No bias init for MSE - default initialization is fine
        ...
        self.softplus = nn.Softplus()  # For non-negative output (matches Model G)
    ...
    def forward(self, images):
        ...
        # Apply Softplus for non-negative output (matches Model G for fair comparison)
        pred = self.softplus(pred)
        return pred


# ============================================================================
# Training with 2um MSE Loss (MASKED) - Matches Model G exactly
# ============================================================================

def train_epoch_mse_2um(model, loader, optimizer, device, grad_accum=1, scaler=None):
    """
    Train for one epoch with MSE loss at 2um ONLY.

    This is the SIMPLE MSE baseline for Model D' - matches Model G exactly.
    - Supervise at 2um (128x128) only
    - Apply mask_2um to only supervise valid tissue regions
    - NO multi-scale pyramid, NO auxiliary losses
    - Uses AMP (Automatic Mixed Precision) for ~2x speedup

    The model outputs Softplus-activated predictions at 2um (128x128).
    We apply mask and compute MSE only on valid spots.
    """
    model.train()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc='Training (D\' Hist2ST + MSE)', leave=False)):
        ...
        with torch.cuda.amp.autocast():
            # Forward: model outputs Softplus predictions at 2um (128x128)
...
def evaluate(model, loader, device):
    ...
    for batch in tqdm(loader, desc='Evaluating', leave=False):
        ...
        # MSE model outputs counts directly (via Softplus), NOT log-rates
        # DO NOT apply exp() here - that's only for Poisson models
        pred_2um = model(images)
...
def main():
    parser = argparse.ArgumentParser(description='Model D\': Hist2ST + Simple 2um MSE (Clean Factorial)')
    ...
    parser.add_argument('--save_dir', type=str,
                        default='/mnt/x/virchow2-decoder-benchmark/results/model_d_prime_hist2st_mse')
    ...
    print(f"\n{'='*60}")
    print(f"MODEL D': Hist2ST + Simple 2um MSE (Clean Factorial)")
    print(f"{'='*60}")
    print(f"PURPOSE: Fair comparison baseline for Hist2ST decoder with MSE loss")
    print(f"Train: {train_patients}, Test: {args.test_patient}")
    print(f"Loss: Simple MSE at 2um with tissue masking (matches Model G)")
    print(f"Output: Softplus activation (matches Model G)")
    print(f"{'='*60}\n")
    ...
    config['model_type'] = 'model_d_prime_hist2st_mse'
    ...
    with torch.no_grad():
        for batch in test_loader:
            images = batch['image'].to(device)
            # MSE model outputs counts directly via Softplus - NO EXP!
            pred_counts = model(images)
            all_pred_2um.append(pred_counts.cpu().numpy())
